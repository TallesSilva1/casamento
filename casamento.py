...
if not st.session_state.invitation_opened:
    ...
    # ── CSS: esconde todo o chrome do Streamlit ──────────────────────
    st.markdown("""
    <style>
    ...
    .hp-topo-eyebrow {
        font-family:'Cinzel',serif; font-size:clamp(8px,1.1vw,10px);
        letter-spacing:5px; color:#c9a84c; text-transform:uppercase; display:block; margin-bottom:5px;
    }
    .hp-topo-nomes {
        font-family:'Dancing Script',cursive;
        font-size:clamp(22px,3.5vw,36px); color:#3a2008; display:block;
    }
    ...
    .hp-ci-local   { font-family:'Cormorant Garamond',serif; font-style:italic; font-size:clamp(9px,1.4vw,13px); color:#7a5c30; margin-bottom:14px; }

    /* Painéis */
    .hp-panel {
        position:absolute; top:0; width:50%; height:100%;
        background:linear-gradient(160deg,#fdfbf0 0%,#f5eecc 55%,#ede4a8 100%);
        transform-style:preserve-3d;
    ...
    .hp-panel-esq { left:0;  transform-origin:left center;  border-radius:8px 0 0 8px; }
    .hp-panel-dir { right:0; transform-origin:right center; border-radius:0 8px 8px 0; }
    .hp-panel-esq::before { content:'';position:absolute;right:0;top:0;width:16px;height:100%;background:linear-gradient(to right,#e8dfa8,#d4c878,#b8a040,#d4c878,#e8dfa8);z-index:2; }
    .hp-panel-dir::before { content:'';position:absolute;left:0;top:0;width:16px;height:100%;background:linear-gradient(to left,#e8dfa8,#d4c878,#b8a040,#d4c878,#e8dfa8);z-index:2; }
    .hp-card.aberto .hp-panel-esq { transform:rotateY(-165deg); }
    .hp-card.aberto .hp-panel-dir { transform:rotateY(165deg); }
    ...
    .hp-lacre-disco { width:68px; height:68px; border-radius:50%; background:radial-gradient(circle at 35% 30%,#f0d060 0%,#c9a030 40%,#a07818 70%,#7a5810 100%); box-shadow:0 0 0 3px #b89028,0 0 0 6px rgba(200,160,40,.22),0 8px 28px rgba(0,0,0,.38),inset 0 2px 8px rgba(255,220,100,.5); }
    .hp-lacre-svg { position:absolute; width:134px; height:134px; top:50%; left:50%; transform:translate(-50%,-50%); animation:rotateSeal 14s linear infinite; pointer-events:none; }
    ...
    /* ── Botão do Streamlit — posicionado sobre o cartão aberto ── */
    ...
    div[data-testid="stButton"].hp-visivel {
        opacity:1 !important;
    ...
    <div class="hp-topo">
        <span class="hp-topo-eyebrow">&#10022; &nbsp; Celebração de Amor &nbsp; &#10022;</span>
        <span class="hp-topo-nomes">Ana Paula &amp; Talles</span>
    </div>

    <div class="hp-stage">
        <div class="hp-card" id="hp-card" onclick="hpAbrirCartao()">
            ...
            <div class="hp-panel hp-panel-esq"></div>
            <div class="hp-panel hp-panel-dir"></div>
            <div class="hp-flores">
                <img src="https://zamgppdvwnzgptoftgta.supabase.co/storage/v1/object/public/photos/image%205%20(1).png" alt="Flores"/>
            </div>
            <div class="hp-lacre">
                <div class="hp-lacre-disco"></div>
                <svg class="hp-lacre-svg" viewBox="0 0 134 134">
                    <path id="hp-cp" d="M67,67 m-45,0 a45,45 0 1,1 90,0 a45,45 0 1,1 -90,0" fill="none"/>
    ...
    function hpAbrirCartao() {
        if (hpAberto) return;
        hpAberto = true;
        var hint = document.getElementById('hp-hint');
        if (hint) hint.classList.add('oculto');
        hpSpawnPetals();
        setTimeout(function() {
            var card = document.getElementById('hp-card');
            if (card) card.classList.add('aberto');
            // Após a animação revelar o interior, mostra o botão do Streamlit
            setTimeout(function() {
    ...
    # Botão real do Streamlit — mostrado pelo JS após o cartão abrir
